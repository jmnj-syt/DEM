...
m_showTexture(true), m_showColorRamp(false),
...
void CTerrainView::OnDraw(CDC* pDC)
{
    ...
    if (m_showColorRamp)
    {
        // 这里需要重新计算顶点颜色，假设已经有了对应的函数
        // 例如，根据高度值查找对应的颜色，并更新顶点数据
        std::vector<float> demData(m_terrainWidth * m_terrainHeight);
        // 重新计算顶点颜色
        for (size_t i = 0; i < m_vertices.size(); i += 5)
        {
            float height = m_vertices[i + 1]; // 获取顶点的高度
            for (size_t j = 0; j < m_colorRampValues.size() - 1; ++j)
            {
                if (height >= m_colorRampValues[j] && height < m_colorRampValues[j + 1])
                {
                    // 为顶点添加颜色属性
                    m_vertices.insert(m_vertices.begin() + i + 3, m_colorRampColors[j].r);
                    m_vertices.insert(m_vertices.begin() + i + 4, m_colorRampColors[j].g);
                    m_vertices.insert(m_vertices.begin() + i + 5, m_colorRampColors[j].b);
                    break;
                }
            }
        }
...
void CTerrainView::LoadDEM(const char* filename)
{
	...
	poBand->RasterIO(GF_Read, 0, 0, m_terrainWidth, m_terrainHeight,
		demData.data(), m_terrainWidth, m_terrainHeight, GDT_Float32, 0, 0);
	...
	for (int y = 0; y < m_terrainHeight - 1; y++) 
	{
		...
		{
			// 第一个三角形 (x,y) -> (x+1,y) -> (x,y+1)
			AddVertex(x, y, demData, scale);
			AddVertex(x + 1, y, demData, scale);
			AddVertex(x, y + 1, demData, scale);

			// 第二个三角形 (x+1,y) -> (x+1,y+1) -> (x,y+1)
			AddVertex(x + 1, y, demData, scale);
			AddVertex(x + 1, y + 1, demData, scale);
			AddVertex(x, y + 1, demData, scale);
		}
	}
...
void CTerrainView::AddVertex(int x, int y, const std::vector<float>& demData, float scale) {
	...
	// 顶点坐标计算
	float xPos = (x - m_terrainWidth / 2.0f) * scale;
	float zPos = (y - m_terrainHeight / 2.0f) * scale;
	float yPos = demData[y * m_terrainWidth + x] * scale * 0.1f;
...
void CTerrainView::OnViewColorRamp()
{
	...
	if (m_showColorRamp) m_showTexture = false;  // 互斥
	Invalidate();  // 刷新视图
}
...
void CTerrainView::ApplyColorRamp(const std::vector<float>& demData)
{
	if (!m_showColorRamp || m_colorRampValues.empty() || m_colorRampColors.empty())
		return;

	// 这里需要根据DEM数据和颜色表来设置顶点颜色
	// 假设顶点数据存储在 m_vertices 中，每个顶点包含 x, y, z 坐标
	for (size_t i = 0; i < m_vertices.size(); i += 3)
	{
		float height = m_vertices[i + 2]; // 获取顶点的高度
		for (size_t j = 0; j < m_colorRampValues.size() - 1; ++j)
		{
			if (height >= m_colorRampValues[j] && height < m_colorRampValues[j + 1])
			{
